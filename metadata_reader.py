...
class MetadataReader:
    def __init__(self, provider, abi, contract, csv_file) -> None:
        ...
        self.metadatas = []
        self.trait_types = []
        self.tokenURIMethod = self.contract.functions['tokenURI']
        ...
        print(f"self.tokenURIMethod = {self.tokenURIMethod}")
        print(f"self.csv_file = {self.csv_file}")

    def get_metadata(self, tokenURI):
        if tokenURI[0:4] == 'ipfs':
            # import pdb; pdb.set_trace()
            request_url = f"https://ipfs.io/ipfs/{tokenURI[7:]}"
        else:
            request_url = tokenURI
    ...
    def process_metadatas_in_increment_of_100(self):
        ...
        try:
            for tokenId in range(self.start_token_id, self.end_token_id):
                tokenURI = self.tokenURIMethod(tokenId).call()
    ...
    def populate_partial_metadatas(self):
        ...
        while self.start_token_id < self.total_supply:
            print ("we're here")
            self.metadatas += self.process_metadatas_in_increment_of_100()
